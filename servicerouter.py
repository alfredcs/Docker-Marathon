...
"""
  Modifed by Alfred on 01-07-2016 to accommodate clustering with HOST networking mode deployment
  ************ Make sure to use keyword "cluster" as the app id and "seed" for the first node in the cluster 
...
def config(apps, groups):
	...
	if ( "cluster" in app.appId ) and ("seed" not in app.appId) :
	  logger.debug("Skip %s due to HOST setting", app.appId)
	else :
	...
	if ( "cluster" in app.appId ) and ("seed" not in app.appId) :
        ...
        for backendServer in sorted(app.backends, key=key_func):
            ...
            serverName = re.sub(
                r'[^a-zA-Z0-9\-]', '_',
                backendServer.host + '_' + (str(backendServer.port) if ("cluster" not in app.appId) else str(app.servicePort)) )
		...
		if ( "cluster" in app.appId ) and ("seed" not in app.appId) :
		  ...
		  if ( "cluster" in app.appId ) and ("seed" in app.appId) :
			#import pdb;pdb.set_trace()
			for appp in sorted(apps, key=attrgetter('appId', 'servicePort')):
			  if ( app.appId.split("/")[1] in appp.appId ) and ( app.servicePort == appp.servicePort ) :
				logger.debug("Server host: %s_%s IP: %s due to HOST setting", appp.appId, appp.servicePort, min(appp.backends).host )
				backends += backend_server_options.format(
				  host=min(appp.backends).host,
                  	...
                  	backends += backend_server_options.format(
                    	  ...
                    	  host_ipv4=ipv4,
                    	  port=app.servicePort if ( "seed" in app.appId ) else backendServer.port,
                    	  serverName=serverName,
                    	  cookieOptions=' check cookie ' +
                    	  serverName if app.sticky else '',
                    	  healthCheckOptions=healthCheckOptions
